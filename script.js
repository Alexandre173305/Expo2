...
window.addEventListener("DOMContentLoaded", () => {
  const music = new Audio("musica-loja.mp3");
  music.loop = true;
  music.volume = 0.4;

  const btnSom = document.createElement("button");
  btnSom.innerText = "🎵 Ativar Música";
  Object.assign(btnSom.style, {
    position: "fixed",
    bottom: "20px",
    right: "20px",
    padding: "10px 18px",
  ...
  btnSom.addEventListener("click", async () => {
    try {
      ...
      console.error(e);
      alert("⚠️ O Edge bloqueou o áudio. Verifique se o arquivo existe e clique novamente.");
    }
  });
});
